...
const getPokemons = async () => {
	...
	const saveDir = process.cwd() + config.imageDestination;
	...
	for(let i = 0; i < data.length; i++) {
		...
		console.log("Saving image of " + pokemon.name + " - " + pokemon.number)
	...
	fs.writeFileSync(process.cwd() + '/pokemons.json', JSON.stringify(pokemons, null, '\t'));
