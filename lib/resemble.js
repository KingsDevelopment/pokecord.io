...
const hash = async (imageUrl, overrideDir = false) => {
	...
	if(configExists) {
		const config = require('../config.json');
		hashDigits = config.resemble.hashDigits;
		imageSize = config.resemble.imageSize;
	}
	...
	// console.log(image);
	// await image.toFile(filename.replace('.png', '_sharp.png'));

	const hash = await imghash.hash(image, hashDigits);
