...
checkPokemonList = async (embed) => {
    ...
    for(let i = 0; i < pokemons.length; i++) {
        ...
        if(exists) {
            if(exists.name !== pokemon.name) {
                console.log("Found number but name is not same, deleting..", number, exists.name, pokemon.name);
        ...
        if(create) {
            ...
            console.log("Creating pokemon!", pokemon);
    ...
    if(data.end < data.total) {
        ...
        client.setTimeout(() => doSync((currentPage + 1)), 2000);
        return;
    }
